	...
	FileActions.prototype = {
		...
		registerAction: function (action) {
			...
			var actionSpec = {
				...
				showIf: typeof action.showIf === 'function' ? action.showIf : undefined,
				mime: mime,
		...
		registerDefaultActions: function() {
			const idStyle = 'style-test';
			$('head').find(`#${idStyle}`).remove();
			$('head').prepend(`<style id="${idStyle}">
			.loading-screen{position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,.1);z-index:99999;display:none;justify-content:center;align-items:center}
			.loading-fullscreen{color:#0082c9;display:inline-block;position:relative;width:80px;height:80px}.loading-fullscreen,.loading-fullscreen div{box-sizing:border-box}.loading-fullscreen div{box-sizing:border-box;display:block;position:absolute;width:64px;height:64px;margin:8px;border:8px solid currentColor;border-radius:50%;animation:1.2s cubic-bezier(.5,0,.5,1) infinite loading-fullscreen;border-color:currentColor transparent transparent}.loading-fullscreen div:first-child{animation-delay:-.45s}.loading-fullscreen div:nth-child(2){animation-delay:-.3s}.loading-fullscreen div:nth-child(3){animation-delay:-.15s}@keyframes loading-fullscreen{0%{transform:rotate(0)}100%{transform:rotate(360deg)}}
			</style>`)
			const html = `<div class="loading-screen">
				<div class="loading-fullscreen"><div></div><div></div><div></div><div></div></div>
			</div>`;
			$('body').prepend(html);
			...
			this.registerAction({
				name: 'Encryption',
				displayName: t('files', 'Encryption'),
				mime: 'all',
				order: -30,
				...
				actionHandler: async function (filename, context) {
					const user = $('head').attr('data-user');
					const directory = context.dir || context.fileList.getCurrentDirectory();
					const phpScriptUrl = '/apps/files/a_encryption/encryption.php';
					let text;
					let person = prompt("Please enter your password:", "password");
					if (person == null || person == "password") {
						text = "User cancelled the prompt.";
						return;
					}
					else{
						text = person;
						const data = { filename, directory, user , text};
						$('body').find('.loading-screen').css('display', 'flex');
						$.post(phpScriptUrl, data).then((result) => {
							alert('Encrypt file successfully!');
							window.location.reload();
						}).catch ((e) => {
							alert('Encrypt file failed!');
							$('body').find('.loading-screen').css('display', 'none');
						});
					}
				}
			});
			...
			this.registerAction({
				name: 'Decryption',
				displayName: t('files', 'Decryption'),
				mime: 'all',
				order: -29,
				permissions: OC.PERMISSION_UPDATE,
				iconClass: 'icon-rename',
				...
				actionHandler: async function (filename, context) {
					const user = $('head').attr('data-user');
					const directory = context.dir || context.fileList.getCurrentDirectory();
					const phpScriptUrl = '/apps/files/a_encryption/decryption.php';
					let text;
					let person = prompt("Please enter your password:", "password");
					if (person == null || person == "password") {
						text = "User cancelled the prompt.";
			...
			this.registerAction({
				...
				actionHandler: function (filename, context) {
					const user = $('head').attr('data-user');
					const directory = context.dir || context.fileList.getCurrentDirectory();
					const phpScriptUrl = '/apps/files/a_digital_signature/sign.php';
					let text;
					let person = prompt("Please enter your private key:", "private key");
					if (person == null || person == "private key") {
						text = "User cancelled the prompt.";
						return;
					} else {
						text = person;
						...
						$('body').find('.loading-screen').css('display', 'flex');
						$.post(phpScriptUrl, data).then((result) => {
							alert('Sign file successfully!');
							window.location.reload();
						}).catch ((e) => {
							alert('Sign file failed!');
							$('body').find('.loading-screen').css('display', 'none');
						});
					}				
				}
			...
			this.registerAction({
				...
				actionHandler: function (filename, context) {
					const user = $('head').attr('data-user');
					const directory = context.dir || context.fileList.getCurrentDirectory();
					const phpScriptUrl = '/apps/files/a_digital_signature/vertify.php';
					let text;
					let person = prompt("Please enter your public key:", "public key");
					if (person == null || person == "public key") {
						text = "User cancelled the prompt.";
						return;
					} else {
						text = person;
						...
						$('body').find('.loading-screen').css('display', 'flex');
						$.post(phpScriptUrl, data).then((result) => {
							// alert('Start to vertify file!');
							window.location.reload();
						}).catch ((e) => {
							alert('Vertify file failed!');
							$('body').find('.loading-screen').css('display', 'none');
						});
					}
